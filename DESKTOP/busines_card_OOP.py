...
class Company:
    ...
    def __str__(self) -> str:
        return f"{self.name} ({self.activity.code})"
...
class VisitCardManager:
    ...
    def search_by_activity(self, activity_code: str) -> List[VisitCard]:
        ...
        return [card for card in self.visit_cards 
                if card.company.activity.code.startswith(activity_code)]
...
if __name__ == "__main__":
    ...
    if activity_a and activity_j:
        ...
        company2 = Company("IT Solutions", activity_j, "+7 (987) 654-32-10", 
        ...
        for card in manager.visit_cards:
            print(f"- {card}: {card.company.activity}")
